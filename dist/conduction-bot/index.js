...
const start = "<b>Уникальность ведения</b> - одновременная работа в двух направлениях – с мамой и с ребёнком. Такой подход гарантирует высокую эффективность и быстрые результаты. 🚀\n" +
    "\n" +
    "<b>Что тебя ждет?</b>\n" +
    "\n" +
    "<b>Для мамы:</b> 👩\n" +
...
conductionBot.start(async (ctx) => {
    try {
        ...
        await ctx.reply("Задайте вопрос в сообщении  или подайте заявку", {
            reply_markup: {
                keyboard: [
                    [
...
conductionBot.command('request', async (ctx) => {
    ...
    if (!check) {
        await ctx.reply('На первом бесплатном созвоне (20–30 минут) мы подробно разберем вашу проблему и я расскажу, чем могу помочь.\n' +
            '\n' +
